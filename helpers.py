...
def prepare_validation_dataset_qa(examples, tokenizer):
    """ Update:
    1. Add the start and end position of the answer
    """
    questions = [q.lstrip() for q in examples["question"]]
    max_seq_length = tokenizer.model_max_length
    tokenized_examples = tokenizer(
    ...
    # include the start and end position of the answer
    tokenized_examples["start_positions"] = []
    tokenized_examples["end_positions"] = []
    ...
    for i, offsets in enumerate(tokenized_examples["offset_mapping"]):
        ...
        if len(answers["answer_start"]) == 0:
            ...
            if not (offsets[token_start_index][0] <= start_char and
                ...
                tokenized_examples["start_positions"].append(
                    token_start_index - 1)
                # print("token_start_index: ", token_start_index - 1)
                while offsets[token_end_index][1] >= end_char:
                    token_end_index -= 1
                tokenized_examples["end_positions"].append(token_end_index + 1)
...
def postprocess_qa_predictions(examples,
    ...
    print("all_start_logits: ", all_start_logits.shape)
    print("all_end_logits: ", all_end_logits.shape)
    ...
    all_predictions: dictionaries mapping example to predicted answer (text)
    training_dynamics: this stores the prediction logits of the start and end positions and also the ground truth start and end positions.
    """
...
# select and extract the predicted answer span from the context.
# Adapted from https://github.com/huggingface/transformers/blob/master/examples/pytorch/question-answering/utils_qa.py
def postprocess_qa_predictions_with_train_dyna(examples,
                                               features,
                                               predictions: Tuple[np.ndarray, np.ndarray],
                                               n_best_size: int = 20):
    if len(predictions) != 2:
    ...
    """
    all_predictions = collections.OrderedDict()
    training_dynamics = collections.OrderedDict()
    ...
    for example_index, example in enumerate(tqdm(examples)):
        ...
        prelim_predictions = []
        start_logits_features = []      # start logits for each feature, muliple features associtated to current example
        end_logits_features = []        # end logits for each feature, muliple features associtated to current example
        ...
        for feature_index in feature_indices:
            ...
            start_logits = all_start_logits[feature_index]
            end_logits = all_end_logits[feature_index]
            start_logits_features.append(start_logits)
            end_logits_features.append(end_logits)
            start_positions = features[feature_index]["start_positions"]
            end_positions = features[feature_index]["end_positions"]
            # This is what will allow us to map some the positions in our logits
            # to span of texts in the original context.
            offset_mapping = features[feature_index]["offset_mapping"]
                    ...
                    prelim_predictions.append(
                        {
                            ...
                            "local_feature_index": local_feature_index,
                        }
                    )
            local_feature_index += 1
        ...
        all_predictions[example["id"]] = predictions[0]["text"]
        training_dynamics[example["id"]] = {
            'gold_start_position': predictions[0]['start_position'],
            # 'gold_end_position': predictions[0]['end_position'],
            'start_logits': start_logits_features[predictions[0]['local_feature_index']],
            # 'end_logits': end_logits_features[predictions[0]['local_feature_index']],
        }

    return all_predictions, training_dynamics
...
class QuestionAnsweringTrainer(Trainer):
    ...
    def evaluate(self,
                 ...
                 evaluate_training_dynamic=False
                 ):
        ...
        if self.compute_metrics is not None:
            ...
            if evaluate_training_dynamic:
                eval_preds, eval_train_dynamic = postprocess_qa_predictions_with_train_dyna(eval_examples,
                                                                                            eval_dataset,
                                                                                            output.predictions)
            else:
        ...
        return metrics, eval_train_dynamic
